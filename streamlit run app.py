...
def add_advanced_stats(df):
    ...
    # 1. 基本フラグの作成（int8型を使ってメモリを極限まで節約）
    df['is_PA'] = np.int8(1)
...
if not include_cs or not include_ns:
    if not df_dates.empty:
        ...
        st.sidebar.caption("※ season_dates.csv が見つからないため、10月中旬以降を簡易除外します。")
...
if target_df.empty:
    ...
    # 🌟構文エラー(SyntaxError)を修正し、正しく安全に計算
    ...
    if total_fair > 0:
        ...
        for d, t in zip(directions, thetas):
            ...
            cnt = len(d_df)
            hit_cnt = len(d_df[d_df['is_H'] == 1]) # その方向への安打数
